...
for i in range(0, 17):
    ...
    path = os.path.join('/home/dicetemp/CITD3/dataset', sessStr, sessStr)
    '''
    for f in ['left', 'right']:
    ...
    for line in fo.readlines():
        y_np = np.append(y_np, np.array([[float(line.split(' ')[3][1:-1]), float(line.split(' ')[4][0:-2])]]), axis=0)
        h_np = np.append(h_np, np.array([[float(line.split(' ')[1][1:-1]), float(line.split(' ')[2][0:-2])]]), axis=0)
